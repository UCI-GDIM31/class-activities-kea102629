...
public class MuskratW7 : MonoBehaviour
{
    ...
    private void MoveOrbitMode()
    {
        ...
        Vector3 up_axis = transform.TransformDirection(Vector3.up);
        transform.RotateAround(_sphereTransform.position,up_axis,leftright*_rotationSpeed*Time.deltaTime);
    ...
    private void MoveNormal()
    {
        ...
        if(Mathf.Abs(_rigidbody.linearVelocity.y)>0.5){
            _animator.SetBool("flying",true);
        }else{
             _animator.SetBool("flying",false);
        }
        if(Mathf.Abs(_rigidbody.linearVelocity.x)>0||Mathf.Abs(_rigidbody.linearVelocity.z)>0){
            _animator.SetBool("running",true);
        }
        else{
